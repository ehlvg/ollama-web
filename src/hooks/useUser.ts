...
import { getApiKey } from "@/lib/web-config";
...
export function useUser() {
  ...
  // For web version, consider authenticated if API key is set
  const isAuthenticated = Boolean(userQuery.data?.name) || Boolean(getApiKey());
