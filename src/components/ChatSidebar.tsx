...
export function ChatSidebar({ currentChatId }: ChatSidebarProps) {
  ...
  const handleRename = useCallback(() => {
    ...
    setContextMenu((prev) => ({ ...prev, isOpen: false }));
  }, [contextMenu.chatId, contextMenu.chatTitle, startEditing]);

  const handleDelete = useCallback(() => {
    handleDeleteChat(contextMenu.chatId);
    setContextMenu((prev) => ({ ...prev, isOpen: false }));
  }, [contextMenu.chatId, handleDeleteChat]);
  ...
  return (
    <nav className="flex flex-1 flex-col min-h-0 select-none">
          ...
          {chatGroups.map((group) => (
            <div key={group.name} className="flex flex-col gap-0.5">
              ...
              {group.chats.map((chat) => (
                <div
                  key={chat.id}
                  className={`flex items-center relative text-sm text-neutral-800 dark:text-neutral-400 rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-800 ${
      ...
      {contextMenu.isOpen && (
        <div
          ref={contextMenuRef}
          className="fixed z-50 min-w-[120px] rounded-lg bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 shadow-lg py-1"
          style={{
        ...
        >
          ...
          <button
            className="w-full px-3 py-2 text-left text-sm text-red-600 dark:text-red-400 hover:bg-neutral-100 dark:hover:bg-neutral-700"
