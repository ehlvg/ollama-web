...
// Cache blob URLs to prevent premature revocation
...
export function ImageThumbnail({
  ...
  const imageUrl = useMemo(() => {
    ...
    const cacheKey = typeof image.data === "string" 
      ? `${image.filename}:${image.data.slice(0,100)}` 
      : `${image.filename}:${(image.data instanceof Uint8Array ? image.data : new Uint8Array(image.data as number[])).length}`;
